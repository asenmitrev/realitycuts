...
export const AdminVideoStats: FC = () => {
  ...
  const totalVideos = data?.reduce((sum, stat) => sum + stat.count, 0) || 0;
  const averageVideos = totalUsers > 0 ? (totalVideos / totalUsers).toFixed(1) : '0';
  const maxVideos = data?.length ?? 0 > 0 ? Math.max(...data!.map(stat => stat.count)) : 0;
        ...
        <Flex>
          <VStack align="start" spacing={2}>
            <Heading size="lg" color="red.400">
              🔒 SECRET ADMIN VIDEO STATS (UAT ONLY)
            </Heading>
            <Text fontSize="sm" color="gray.500">
              Shows video creation statistics by user (excluding founders) for the last {days} days
            </Text>
          </VStack>
          <Spacer />
          <HStack>
          ...
          <Card>
            <CardBody>
              <Stat>
                ...
                <StatHelpText>Users with videos</StatHelpText>
              </Stat>
            </CardBody>
          </Card>
          <Card>
            <CardBody>
              <Stat>
                ...
                <StatNumber>{totalVideos}</StatNumber>
          ...
          <Card>
            <CardBody>
              <Stat>
                ...
                <StatNumber>{averageVideos}</StatNumber>
                <StatHelpText>Videos per user</StatHelpText>
              </Stat>
            </CardBody>
          </Card>
          <Card>
            <CardBody>
              <Stat>
                ...
                <StatNumber>{maxVideos}</StatNumber>
                <StatHelpText>Max videos</StatHelpText>
              </Stat>
            </CardBody>
          </Card>
        </SimpleGrid>
          ...
          <Box overflowX="auto">
            ...
            {data && data.length === 0 && (
              <VStack py={8} spacing={4}>
                <Text color="gray.500">No video data found for the last {days} days</Text>
              </VStack>
            )}
          </Box>
        )}

...
export const AdminLibraryStats: FC = () => {
  ...
  const { isLoading, data, error, refetch } = useQuery({
    queryKey: ['adminLibraryStats', days],
    queryFn: async () => {
      try {
  ...
  const totalLibraries = data?.reduce((sum, stat) => sum + stat.count, 0) || 0;
  const averageLibraries = totalUsers > 0 ? (totalLibraries / totalUsers).toFixed(1) : '0';
  const maxLibraries = data?.length ?? 0 > 0 ? Math.max(...data!.map(stat => stat.count)) : 0;
        ...
        <Flex>
          <VStack align="start" spacing={2}>
            <Heading size="lg" color="purple.400">
              🔒 SECRET ADMIN LIBRARY STATS (UAT ONLY)
            </Heading>
            <Text fontSize="sm" color="gray.500">
              Shows library creation statistics by user (excluding founders) for the last {days} days
            </Text>
          </VStack>
          <Spacer />
          <HStack>
          ...
          <Card>
            <CardBody>
              <Stat>
                ...
                <StatHelpText>Users with libraries</StatHelpText>
              </Stat>
            </CardBody>
          </Card>
          <Card>
            <CardBody>
              <Stat>
                <StatLabel>Total Libraries</StatLabel>
                <StatNumber>{totalLibraries}</StatNumber>
          ...
          <Card>
            <CardBody>
              <Stat>
                ...
                <StatNumber>{averageLibraries}</StatNumber>
                <StatHelpText>Libraries per user</StatHelpText>
              </Stat>
            </CardBody>
          </Card>
          <Card>
            <CardBody>
              <Stat>
                ...
                <StatNumber>{maxLibraries}</StatNumber>
                <StatHelpText>Max libraries</StatHelpText>
              </Stat>
            </CardBody>
          </Card>
        </SimpleGrid>
          ...
          <Box overflowX="auto">
            ...
            {data && data.length === 0 && (
              <VStack py={8} spacing={4}>
                <Text color="gray.500">No library data found for the last {days} days</Text>
              </VStack>
            )}
          </Box>
        )}

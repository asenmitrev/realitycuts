...
export const useBrollUnknown = (libraryId: string | undefined, currentPage: number) => {
  ...
  const {
    ...
    queryKey: ['library', libraryId, 'broll', 'unknown', currentPage],
    queryFn: async () => {
      const response = await apiService.get<BrollDataResponse>(
        `/api/library/${libraryId}/broll/unknown?skip=${currentPage * BROLL_PER_PAGE}&limit=${BROLL_PER_PAGE}`
      );
      return response;
    },
    enabled: !!libraryId

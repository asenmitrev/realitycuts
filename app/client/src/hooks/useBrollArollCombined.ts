...
export const useBrollArollCombined = (libraryId: string | undefined, currentPage: number) => {
  ...
  const {
    ...
    queryKey: ['library', libraryId, 'broll', 'aroll-combined', currentPage],
    queryFn: async () => {
      const response = await apiService.get<BrollDataResponse>(
        `/api/library/${libraryId}/broll/aroll-combined?skip=${currentPage * BROLL_PER_PAGE}&limit=${BROLL_PER_PAGE}`
      );
      return response;
    },
    enabled: !!libraryId

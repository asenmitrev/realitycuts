...
Full-stack web application for AI-powered video generation and editing. Users upload video libraries, generate videos from prompts using LLMs, edit in a canvas-based editor, and export/download the finished video. There is no TikTok/Instagram publishing integration on this self-hosted branch — those require ongoing OAuth app review from each platform, which isn't feasible to maintain for an open-source project. Direct-to-YouTube upload (`app/server/services/youtube-upload.service.ts`, mounted at `/api/youtube`) is supported for a single self-hosted instance: the OAuth redirect URI is `${API_URL}/api/youtube/callback`, which is `http://localhost:3021/...` by default, so no public URL/tunnel is needed. Keep the Google OAuth consent screen in "Testing" mode with your own account as a test user — that avoids Google's review process, at the cost of refresh tokens expiring after 7 days (re-connect from the YouTube settings page when that happens).
